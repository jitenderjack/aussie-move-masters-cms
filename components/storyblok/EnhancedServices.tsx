        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blok.services?.map((service) => {
            ...
            const badgeColors: Record<string, string> = {
              popular: "bg-[hsl(37,91%,55%)] text-gray-900",
            ...
            return (
              <div key={service._uid} className="relative bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all border border-gray-100 group">
                ...
                <div className="text-[hsl(174,65%,28%)] mb-4 mt-2">
                  <Icon className="w-12 h-12" />
                </div>
                <h3 className="text-xl font-black text-gray-900 mb-3">{service.title}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">{service.description}</p>
                <button className="inline-flex items-center gap-2 text-[hsl(174,65%,28%)] font-semibold text-sm hover:gap-3 transition-all group-hover:text-[hsl(37,91%,55%)]">
                  Get a Quote
                  <span>→</span>
                </button>
              </div>
