...
export default function Footer({ blok }: FooterProps) {
  return (
    <footer {...storyblokEditable(blok)} className="bg-[hsl(174,65%,20%)] text-white">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid md:grid-cols-3 gap-10 mb-10">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-10 h-10 bg-[hsl(174,65%,28%)] rounded-lg flex items-center justify-center">
            ...
            <div className="flex gap-3">
              <a
                ...
                className="flex items-center gap-1.5 bg-[hsl(37,91%,55%)] text-gray-900 font-bold px-4 py-2 rounded-full text-xs hover:bg-[hsl(37,91%,48%)] transition-colors font-['Montserrat']"
              >
              ...
              {blok.nav_items?.map((item) => (
                <li key={item._uid}>
                  <button
                    ...
                    className="text-sm opacity-70 hover:opacity-100 hover:text-[hsl(37,91%,55%)] transition-all font-['Montserrat']"
                  >
                    {item.label}
                  </button>
                </li>
          ...
          <div>
            ...
            <div className="space-y-3 text-sm opacity-75">
              <a
                href={`tel:${blok.phone.replace(/\s/g, "")}`}
                className="flex items-center gap-2 hover:opacity-100 hover:text-[hsl(37,91%,55%)] transition-all"
              >
